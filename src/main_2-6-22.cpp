...
//Forward kinematics to determine the current shape
    double R = 30.0, s1 = 680.72, s2 = 698.5;
    // for(int i=0;i<3;i++){
...
void inv_kinematics_not_moment(double k1,double phi1, double k2, double phi2){
    double R = 30.0, s1 = 680.72,s2 = 698.5;
    double ref_retraction = -s2 * R * k2; //x when it is at tendon midplanes
...
void inv_kinematics_moments_mod(double k1,double phi1, double k2, double phi2){
    double R = 30.0, s1 = 680.72,s2 = 698.5;
    double ref_retraction1 = -s1 * R *k1;
    // double ref_retraction2 = -s2 * R * k2;
    // double ref_retraction2 = -s2 * R * k2 * (1 - 0.275*sin(1.5*fmod(phi2-PI/3,2*PI/3)));
    double ref_retraction2 = -s2 * R * k2 * (1 - 0.325*sin(1.5*fmod(phi2-PI/3.0,2.0*PI/3.0)));
    // double ref_retraction2 = -s2 * R * k2 * (1 - 0.325*sin(3*fmod(phi2-PI/3,PI/3)));
    phi1=wrapAngle(phi1);
    phi2 = wrapAngle(phi2);
    // double ref_retraction1 = -s1 * R *k1;
    // double ref_retraction2 = -s2 * R * k2 * (1 - 0.275*sin(1.5*fmod(phi2,2*PI/3)));
    cap_lengths[0] = ((cos(phi1 - PI/6)*sin(phi1 + PI/6) - cos(phi1 + PI/6)*sin(phi1 - PI/6) + cos(phi1 + PI/6)*heaviside(phi1 - (4*PI)/3)*sin(phi1 - PI/6) - heaviside((2*PI)/3 - phi1)*heaviside(phi1)*cos(phi1 - PI/6)*sin(phi1 + PI/6))/(sin(phi1)*heaviside(phi1 - (4*PI)/3)*sin(phi1 - PI/6)*sin(phi1 + PI/6) + cos(phi1)*cos(phi1 + PI/6)*heaviside(phi1 - (4*PI)/3)*sin(phi1 - PI/6) - heaviside((2*PI)/3 - phi1)*cos(phi1)*heaviside(phi1)*cos(phi1 - PI/6)*sin(phi1 + PI/6) + heaviside((4*PI)/3 - phi1)*cos(phi1)*cos(phi1 - PI/6)*heaviside(phi1 - (2*PI)/3)*sin(phi1 + PI/6) - heaviside((4*PI)/3 - phi1)*cos(phi1)*cos(phi1 + PI/6)*heaviside(phi1 - (2*PI)/3)*sin(phi1 - PI/6) - heaviside((2*PI)/3 - phi1)*heaviside(phi1)*sin(phi1)*sin(phi1 - PI/6)*sin(phi1 + PI/6)) - (cos(phi1 - PI/6)*sin(phi1 + PI/6) - cos(phi1 + PI/6)*sin(phi1 - PI/6))/(sin(phi1)*heaviside(phi1 - (4*PI)/3)*sin(phi1 - PI/6)*sin(phi1 + PI/6) + cos(phi1)*cos(phi1 + PI/6)*heaviside(phi1 - (4*PI)/3)*sin(phi1 - PI/6) - heaviside((2*PI)/3 - phi1)*cos(phi1)*heaviside(phi1)*cos(phi1 - PI/6)*sin(phi1 + PI/6) + heaviside((4*PI)/3 - phi1)*cos(phi1)*cos(phi1 - PI/6)*heaviside(phi1 - (2*PI)/3)*sin(phi1 + PI/6) - heaviside((4*PI)/3 - phi1)*cos(phi1)*cos(phi1 + PI/6)*heaviside(phi1 - (2*PI)/3)*sin(phi1 - PI/6) - heaviside((2*PI)/3 - phi1)*heaviside(phi1)*sin(phi1)*sin(phi1 - PI/6)*sin(phi1 + PI/6)))*ref_retraction1;
...
// if (phi2 > PI/3 - PI/6 && phi2 < PI/3 + PI/6){
//     cap_lengths[3] = 1.1547*ref_retraction2;  
// }
// if (phi2 > PI - PI/6 && phi2 < PI + PI/6){
//     cap_lengths[4] = 1.1547*ref_retraction2;  
// }
// if (phi2 > 5*PI/3 - PI/6 && phi2 < 5*PI/3 + PI/6){
//     cap_lengths[5] = 1.1547*ref_retraction2;  
// }

// if (phi2 > PI/3 - PI/6 && phi2 < PI/3 + PI/6){
//     if (phi2<PI/3){
//         cap_lengths[3] = (1.37 + 0.5*(phi2 - PI/3))*ref_retraction2;
//     }else{
//         cap_lengths[3] = (1.37 - 0.5*(phi2 - PI/3))*ref_retraction2;
//     }
// }
// if (phi2 > PI - PI/6 && phi2 < PI + PI/6){
//     if (phi2<PI){
//         cap_lengths[4] = (1.37 + 0.5*(phi2 - PI))*ref_retraction2;
//     }else{
//         cap_lengths[4] = (1.37 - 0.5*(phi2 - PI))*ref_retraction2;
//     }
// }
// if (phi2 > 5*PI/3 - PI/6 && phi2 < 5*PI/3 + PI/6){
//     if (phi2<5*PI/3){
//         cap_lengths[5] = (1.37 + 0.5*(phi2 - 5*PI/3))*ref_retraction2;
//     }else{
//         cap_lengths[5] = (1.37 - 0.5*(phi2 - 5*PI/3))*ref_retraction2;
//     } 
// }
...
void inv_kinematics_mod_cosine(double k1,double phi1, double k2, double phi2){
double R = 30.0, s1 = 680.72,s2 = 698.5;
    double ref_retraction = -s2 * R * k2;
    phi2 = wrapAngle(phi2);
    ...
    if(PI/3.0 <= phi2 && phi2 <= PI){
        cap_lengths[3] = cos(3.0/4.0*(PI/3.0 - phi2))*ref_retraction;
        cap_lengths[4] = cos(3.0/4.0*(PI - phi2))*ref_retraction;
    }else if (PI < phi2 && phi2 <= 5.0*PI/3.0){
        cap_lengths[4] = cos(3.0/4.0*(PI - phi2))*ref_retraction;
        cap_lengths[5] = cos(3.0/4.0*(5.0*PI/3.0 - phi2))*ref_retraction;
    }else if (5.0*PI/3.0 < phi2 && phi2 <= 2.0*PI){
        cap_lengths[3] = cos(3.0/4.0*(PI/3.0 - phi2)+ 3.0*PI/2.0)*ref_retraction;
        cap_lengths[5] = cos(3.0/4.0*(5.0*PI/3.0 - phi2))*ref_retraction;
    } else{
        cap_lengths[3] = cos(3.0/4.0*(PI/3.0 - phi2))*ref_retraction;
        cap_lengths[5] = cos(3.0/4.0*(5.0*PI/3.0 - phi2) -3.0*PI/2.0)*ref_retraction;
    }
}
void elliptical_tendon_lengths(double k1,double phi1, double k2, double phi2){
  double R = 30.0, s1 = 680.72,s2 = 698.5;
  double ref_retraction1 = -s1 * R *k1;
  double ref_retraction2 = -80;//-s2 * R * k2;// *(1 - 0.325*sin(1.5*fmod(phi2-PI/3.0,2.0*PI/3.0)));
  phi1=wrapAngle(phi1);
  phi2 = wrapAngle(phi2);
  cap_lengths[0] = 0;
    cap_lengths[1] = 0;
    ...
    // if(PI/3.0 <= phi2 && phi2 <= PI){
    //     cap_lengths[3] = pow(1.0-pow(3.0*(phi2-PI/3.0)/(2.0*PI),2.0),0.5)*ref_retraction2;
    //     cap_lengths[4] = pow(1.0-pow(3.0*(phi2-PI)/(2.0*PI),2.0),0.5)*ref_retraction2;
    // }else if (PI < phi2 && phi2 <= 5.0*PI/3.0){
    //     cap_lengths[4] = pow(1.0-pow(3.0*(phi2-PI)/(2.0*PI),2.0),0.5)*ref_retraction2;
    //     cap_lengths[5] = pow(1.0-pow(3.0*(phi2-5.0*PI/3.0)/(2.0*PI),2.0),0.5)*ref_retraction2;
    // }else if (5.0*PI/3.0 < phi2 && phi2 <= 2.0*PI){
    //     cap_lengths[3] = pow(1.0-pow(3.0*(phi2 -2.0*PI -PI/3.0)/(2.0*PI),2.0),0.5)*ref_retraction2;
    //     cap_lengths[5] = pow(1.0-pow(3.0*(phi2-5.0*PI/3.0)/(2.0*PI),2.0),0.5)*ref_retraction2;
    // } else{
    //     cap_lengths[3] = pow(1.0-pow(3.0*(phi2-PI/3.0)/(2.0*PI),2.0),0.5)*ref_retraction2;
    //     cap_lengths[5] = pow(1.0-pow(3.0*(phi2 + 2.0*PI -5.0*PI/3.0)/(2.0*PI),2.0),0.5)*ref_retraction2;
    // }
    // if(PI/3.0 <= phi2 && phi2 <= PI){
    //     cap_lengths[3] = sqrt(1.0-pow(3.0*(phi2-PI/3.0)/(2.0*PI),2.0))*ref_retraction2;
    //     cap_lengths[4] = sqrt(1.0-pow(3.0*(phi2-PI)/(2.0*PI),2.0))*ref_retraction2;
    //     cap_lengths[5] = 0;
    // }else if (PI < phi2 && phi2 <= 5.0*PI/3.0){
    //     cap_lengths[3] = 0;
    //     cap_lengths[4] = sqrt(1.0-pow(3.0*(phi2-PI)/(2.0*PI),2.0))*ref_retraction2;
    //     cap_lengths[5] = sqrt(1.0-pow(3.0*(phi2-5.0*PI/3.0)/(2.0*PI),2.0))*ref_retraction2;
    // }else if (5.0*PI/3.0 < phi2 && phi2 <= 2.0*PI){
    //     cap_lengths[3] = sqrt(1.0-pow(3.0*(phi2 -2.0*PI -PI/3.0)/(2.0*PI),2.0))*ref_retraction2;
    //     cap_lengths[4] = 0;
    //     cap_lengths[5] = sqrt(1.0-pow(3.0*(phi2-5.0*PI/3.0)/(2.0*PI),2.0))*ref_retraction2;
    // } else{
    //     cap_lengths[3] = sqrt(1.0-pow(3.0*(phi2-PI/3.0)/(2.0*PI),2.0))*ref_retraction2;
    //     cap_lengths[4] = 0;
    //     cap_lengths[5] = sqrt(1.0-pow(3.0*(phi2 + 2.0*PI -5.0*PI/3.0)/(2.0*PI),2.0))*ref_retraction2;
    // }
...
// double width_control = 2.0*PI/3.0 + PI/6;//PI/128000.0; 
//         if(PI/3.0 <= phi2 && phi2 <= PI){
//         cap_lengths[3] = sqrt(1.0-pow((phi2-PI/3.0)/width_control,2.0))*ref_retraction2;
//         cap_lengths[4] = sqrt(1.0-pow((phi2-PI)/width_control,2.0))*ref_retraction2;
//         cap_lengths[5] = 0;
//     }else if (PI < phi2 && phi2 <= 5.0*PI/3.0){
//         cap_lengths[3] = 0;
//         cap_lengths[4] = sqrt(1.0-pow((phi2-PI)/width_control,2.0))*ref_retraction2;
//         cap_lengths[5] = sqrt(1.0-pow((phi2-5.0*PI/3.0)/width_control,2.0))*ref_retraction2;
//     }else if (5.0*PI/3.0 <= phi2 && phi2 <= 2.0*PI){
//         cap_lengths[3] = sqrt(1.0-pow((phi2 -2.0*PI -PI/3.0)/width_control,2.0))*ref_retraction2;
//         cap_lengths[4] = 0;
//         cap_lengths[5] = sqrt(1.0-pow((phi2-5.0*PI/3.0)/width_control,2.0))*ref_retraction2;
//     } else{
//         cap_lengths[3] = sqrt(1.0-pow((phi2-PI/3.0)/width_control,2.0))*ref_retraction2;
//         cap_lengths[4] = 0;
//         cap_lengths[5] = sqrt(1.0-pow((phi2 + 2.0*PI -5.0*PI/3.0)/width_control,2.0))*ref_retraction2;
//     }

// equal width adjustment with return to zero blocked
double width_control = 2.0*PI/3.0 + PI/20.0;//PI/72.0; 
        if(PI/3.0 <= phi2 && phi2 <= PI){
        cap_lengths[3] = sqrt(1.0-pow((phi2-PI/3.0)/width_control,2.0))*ref_retraction2;
        if (phi2<2*PI/3){
            cap_lengths[4] = cap_lengths[4]>sqrt(1.0-pow((phi2-PI)/width_control,2.0))*ref_retraction2?sqrt(1.0-pow((phi2-PI)/width_control,2.0))*ref_retraction2:cap_lengths[4];
        }else{
            cap_lengths[4] = sqrt(1.0-pow((phi2-PI)/width_control,2.0))*ref_retraction2;
        }
        // cap_lengths[5] = 0;
        ...
        cap_lengths[4] = sqrt(1.0-pow((phi2-PI)/width_control,2.0))*ref_retraction2;
        if (phi2<4*PI/3){
            cap_lengths[5] = cap_lengths[5]>sqrt(1.0-pow((phi2-5.0*PI/3.0)/width_control,2.0))*ref_retraction2?sqrt(1.0-pow((phi2-5.0*PI/3.0)/width_control,2.0))*ref_retraction2:cap_lengths[5];
        }else{
            cap_lengths[5] = sqrt(1.0-pow((phi2-5.0*PI/3.0)/width_control,2.0))*ref_retraction2;
        }
        ...
        if(phi2<2*PI){
            cap_lengths[3] = cap_lengths[3]>sqrt(1.0-pow((phi2 -2.0*PI -PI/3.0)/width_control,2.0))*ref_retraction2?sqrt(1.0-pow((phi2 -2.0*PI -PI/3.0)/width_control,2.0))*ref_retraction2:cap_lengths[3];
        }else{
            cap_lengths[3] = sqrt(1.0-pow((phi2 -2.0*PI -PI/3.0)/width_control,2.0))*ref_retraction2;
        }
...
//skewed width adjustment - jitters in the tendon planes and not interesting
    // double width_control = 2.0*PI/3.0 + PI/36.0;
    //     if(PI/3.0 <= phi2 && phi2 <= PI){
    //     cap_lengths[3] = sqrt(1.0-pow((phi2-PI/3.0)/width_control,2.0))*ref_retraction2;
    //     cap_lengths[4] = sqrt(1.0-pow((phi2-PI)/(2.0*PI/3.0),2.0))*ref_retraction2;
    //     // cap_lengths[5] = 0;
    // }else if (PI < phi2 && phi2 <= 5.0*PI/3.0){
    //     // cap_lengths[3] = 0;
    //     cap_lengths[4] = sqrt(1.0-pow((phi2-PI)/width_control,2.0))*ref_retraction2;
    //     cap_lengths[5] = sqrt(1.0-pow((phi2-5.0*PI/3.0)/(2.0*PI/3.0),2.0))*ref_retraction2;
    // }else if (5.0*PI/3.0 <= phi2 && phi2 <= 2.0*PI){
    //     cap_lengths[3] = sqrt(1.0-pow((phi2 -2.0*PI -PI/3.0)/(2.0*PI/3.0),2.0))*ref_retraction2;
    //     // cap_lengths[4] = 0;
    //     cap_lengths[5] = sqrt(1.0-pow((phi2-5.0*PI/3.0)/width_control,2.0))*ref_retraction2;
    // } else{
    //     cap_lengths[3] = sqrt(1.0-pow((phi2-PI/3.0)/(2.0*PI/3.0),2.0))*ref_retraction2;
    //     // cap_lengths[4] = 0;
    //     cap_lengths[5] = sqrt(1.0-pow((phi2 + 2.0*PI -5.0*PI/3.0)/width_control,2.0))*ref_retraction2;
    // }
}
void quadratic_tendon_lengths(double k1,double phi1, double k2, double phi2){
  double R = 30.0, s1 = 680.72,s2 = 698.5;
  double ref_retraction1 = -s1 * R *k1;
  double ref_retraction2 = -s2 * R * k2;// *(1 - 0.325*sin(1.5*fmod(phi2-PI/3.0,2.0*PI/3.0)));
  phi1=wrapAngle(phi1);
  ...
  cap_lengths[2] = 0;
  if(PI/3.0 <= phi2 && phi2 <= PI){
      cap_lengths[3] = (1.0-pow(3.0*(phi2-PI/3.0)/(2.0*PI),2.0))*ref_retraction2;
      cap_lengths[4] = (1.0-pow(3.0*(phi2-PI)/(2.0*PI),2.0))*ref_retraction2;
      cap_lengths[5] = 0;
  }else if (PI < phi2 && phi2 <= 5.0*PI/3.0){
      cap_lengths[3] = 0;
      cap_lengths[4] = (1.0-pow(3.0*(phi2-PI)/(2.0*PI),2.0))*ref_retraction2;
      cap_lengths[5] = (1.0-pow(3.0*(phi2-5.0*PI/3.0)/(2.0*PI),2.0))*ref_retraction2;
  }else if (5.0*PI/3.0 < phi2 && phi2 <= 2.0*PI){
      cap_lengths[3] = (1.0-pow(3.0*(phi2 -2.0*PI -PI/3.0)/(2.0*PI),2.0))*ref_retraction2;
      cap_lengths[4] = 0;
      cap_lengths[5] = (1.0-pow(3.0*(phi2-5.0*PI/3.0)/(2.0*PI),2.0))*ref_retraction2;
  } else{
      cap_lengths[3] = (1.0-pow(3.0*(phi2-PI/3.0)/(2.0*PI),2.0))*ref_retraction2;
      cap_lengths[4] = 0;
      cap_lengths[5] = (1.0-pow(3.0*(phi2 + 2.0*PI -5.0*PI/3.0)/(2.0*PI),2.0))*ref_retraction2;
  }
}
void quadratic_elliptical_tendon_lengths(double k1,double phi1, double k2, double phi2){
 double R = 30.0, s1 = 680.72,s2 = 698.5;
  double ref_retraction1 = -s1 * R *k1;
  double ref_retraction2 = -s2 * R * k2;// *(1 - 0.325*sin(1.5*fmod(phi2-PI/3.0,2.0*PI/3.0)));
  double a = 0.6;
  ...
  cap_lengths[0] = 0;
    ...
    cap_lengths[2] = 0;
        if(PI/3.0 <= phi2 && phi2 <= PI){
        cap_lengths[3] = sqrt(1.0-pow(3.0*(phi2-PI/3.0)/(2.0*PI),2.0))*ref_retraction2*a + (1.0-pow(3.0*(phi2-PI/3.0)/(2.0*PI),2.0))*ref_retraction2*(1-a);
        cap_lengths[4] = sqrt(1.0-pow(3.0*(phi2-PI)/(2.0*PI),2.0))*ref_retraction2*a + (1.0-pow(3.0*(phi2-PI)/(2.0*PI),2.0))*ref_retraction2*(1-a);
        cap_lengths[5] = 0;
    }else if (PI <= phi2 && phi2 <= 5.0*PI/3.0){
        cap_lengths[3] = 0;
        cap_lengths[4] = sqrt(1.0-pow(3.0*(phi2-PI)/(2.0*PI),2.0))*ref_retraction2*a + (1.0-pow(3.0*(phi2-PI)/(2.0*PI),2.0))*ref_retraction2*(1-a);
        cap_lengths[5] = sqrt(1.0-pow(3.0*(phi2-5.0*PI/3.0)/(2.0*PI),2.0))*ref_retraction2*a+ (1.0-pow(3.0*(phi2-5.0*PI/3.0)/(2.0*PI),2.0))*ref_retraction2*(1-a);
    }else if (5.0*PI/3.0 <= phi2 && phi2 <= 2.0*PI){
        cap_lengths[3] = sqrt(1.0-pow(3.0*(phi2 -2.0*PI -PI/3.0)/(2.0*PI),2.0))*ref_retraction2*a + (1.0-pow(3.0*(phi2 -2.0*PI -PI/3.0)/(2.0*PI),2.0))*ref_retraction2*(1-a);
        cap_lengths[4] = 0;
        cap_lengths[5] = sqrt(1.0-pow(3.0*(phi2-5.0*PI/3.0)/(2.0*PI),2.0))*ref_retraction2*a + (1.0-pow(3.0*(phi2-5.0*PI/3.0)/(2.0*PI),2.0))*ref_retraction2*(1-a);
    } else{
        cap_lengths[3] = sqrt(1.0-pow(3.0*(phi2-PI/3.0)/(2.0*PI),2.0))*ref_retraction2*a + (1.0-pow(3.0*(phi2-PI/3.0)/(2.0*PI),2.0))*ref_retraction2*(1-a);
        cap_lengths[4] = 0;
        cap_lengths[5] = sqrt(1.0-pow(3.0*(phi2 + 2.0*PI -5.0*PI/3.0)/(2.0*PI),2.0))*ref_retraction2*a + (1.0-pow(3.0*(phi2 + 2.0*PI -5.0*PI/3.0)/(2.0*PI),2.0))*ref_retraction2*(1-a);
    }
...
void loop(){
    ...
    if(current_time - last_waypoint_time >= waypoint_interval){
        ...
        target_phi1 = target_phi1 + PI/3;
        // Serial.print("Global variable target phi1: ");
        // Serial.print(target_phi1);
        // Serial.print('\t');
        target_phi2 = PI + target_phi1;
        // Serial.print("Global variable target phi2: ");
        // Serial.println(target_phi2);
        // interpolate_jones_space(0.5*PI/680.72,target_phi1,0.5*PI/698.5,target_phi2);
        // interpolate_jones_space(0.8*PI/680.72,target_phi1,0.8*PI/698.5,target_phi2);
        // interpolate_jones_space(0.0,target_phi1,0.5*PI/698.5,target_phi2);
        interpolate_jones_space(0.0,target_phi1,0.0,target_phi2);
        // interpolate_jones_space(0.5*PI/680.72,target_phi1,0.0,target_phi2);
        // interpolate_jones_space(0.0,target_phi1,0.5*PI*(1.0+0.5*sin(7.0*phi2))/698.5,target_phi2);
